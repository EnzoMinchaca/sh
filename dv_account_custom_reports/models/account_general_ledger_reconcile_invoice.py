# -*- coding: utf-8 -*-
# Informe de conciliación Invoices
import json
...
class ReconcileInvoiceLedgerCustomHandler(models.AbstractModel):    # Informe de conciliación Invoices
    _name = 'account.reconcile.invoice.ledger.report.handler'
    _description = 'Informe de conciliación Invoices'
    ...
    def _query_values(self, report, options):
        ...
        if groupby_accounts:
            ...
            accounts = []
        specific_codes = [
            '52101005', '54201005', '54201505', '54201515', 
            '54201905', '52101015', '52101505', '55501515'
        ]
    ...
    def _get_query_sums(self, report, options):
        ...
        for column_group_key, options_group in options_by_column_group.items():
            ...
            queries.append(f"""
                ...
                WHERE {where_clause}
                AND account_move_line.td_invoice IS NOT NULL
    ...
    def _get_query_amls(self, report, options, expanded_account_ids, offset=0, limit=None):
        ...
        for column_group_key, group_options in report._split_options_per_column_group(options).items():
            ...
            query = f'''
                ...
                WHERE {where_clause}
                AND account_move_line.td_invoice IS NOT NULL
                AND account_move_line.td_invoice != ''
    ...
    def _report_expand_unfoldable_line_general_ledger(self, line_dict_id, groupby, options, progress, offset, unfold_all_batch_data=None):
        ...
        # Obtener las líneas previamente almacenadas en el contexto
        context_key = f'expandable_lines_{line_dict_id}'  # Clave única para identificar esta expansión
        previous_lines_snapshot = self.env.context.get(context_key, [])  # Obtener del contexto si existe
        ...
        # Las líneas de nivel 2 tienen el valor de td_invoice directamente en 'td_invoice', mientras que las de nivel 3 están en 'columns'
        combined_lines = sorted(
            level_2_lines + lines, 
            key=lambda l: self._get_td_invoice_from_columns_or_level_2(l)
        )

        # Identificar los 'td_invoice' que deben eliminarse
        td_invoice_to_remove = set()
        ...
        for line in level_2_lines:
            if line.get('level') == 2:
                ...
                # Si balance o balance_usd es 0, marcar 'td_invoice' para eliminación
                if balance == 0 and balance_usd == 0:
                    td_invoice_to_remove.add(self._get_td_invoice_from_columns_or_level_2(line))
        
        # Filtrar las líneas eliminando las que tengan 'td_invoice' en la lista de eliminación
        final_lines = []
        last_level_2_id = None
        for line in combined_lines:
            if line.get('level') == 2:
                td_invoice = line.get('td_invoice', '')
                if td_invoice in td_invoice_to_remove:
                    continue  # No agregar la línea de nivel 2 si debe ser eliminada
            
            # Para las líneas de nivel 3, verificar si su 'td_invoice' debe ser eliminado
            if line.get('level') == 3:
                columns = line.get('columns', [])
                td_invoice = self._get_td_invoice_from_columns(columns)
                if td_invoice in td_invoice_to_remove:
                    continue  # No agregar la línea de nivel 3 si su 'td_invoice' debe ser eliminado
            
            final_lines.append(line)

        # Agregar las nuevas líneas a las líneas previamente almacenadas
        accumulated_lines = previous_lines_snapshot + final_lines

        # Actualizar el contexto con las nuevas líneas acumuladas
        self = self.with_context({context_key: accumulated_lines})
    ...
    def _get_td_invoice_from_columns_or_level_2(self, line):
        """
        Extrae el valor de 'td_invoice' para las líneas de nivel 2 y 3.
        Si es una línea de nivel 2, utiliza el campo 'td_invoice' directamente.
        Si es una línea de nivel 3, lo busca dentro de las columnas.
        """
        # Si es nivel 2, el td_invoice está en la propia línea
        if line.get('level') == 2:
            return line.get('td_invoice', '')

        # Si es nivel 3, el td_invoice está en las columnas
        return self._get_td_invoice_from_columns(line.get('columns', []))
    
    def _get_td_invoice_from_columns(self, columns):
        """
        Extrae el valor de 'td_invoice' desde las columnas si está presente.
        """
        for col in columns:
            if col.get('expression_label') == 'td_invoice':
    ...
    def _generate_level_2_lines(self, aml_results, report, parent_line_id, columns, options):
        ...
        # Iterar sobre los resultados para agrupar por td_invoice
        for aml_key, aml_data in aml_results.items():
            ...
            for key, value in aml_data.items():
                if isinstance(value, dict):
                    td_invoice = value.get('td_invoice')
                ...
                # Si no encontramos td_invoice en los datos, continuamos al siguiente resultado
                continue
            
            if td_invoice not in grouped_lines:
                grouped_lines[td_invoice] = {
                    'total_debit': 0,
                    'total_credit': 0,
                    'total_balance': 0,
                    'total_balance_usd': 0,
            ...
            grouped_lines[td_invoice]['total_debit'] += value.get('debit', 0)
            grouped_lines[td_invoice]['total_credit'] += value.get('credit', 0)
            grouped_lines[td_invoice]['total_balance'] += balance
            grouped_lines[td_invoice]['total_balance_usd'] += balance_usd

        # Crear líneas de nivel 2
        for td_invoice, group_data in grouped_lines.items():
            
            level_2_line = {
                'id': report._get_generic_line_id('account.move.line', f"{td_invoice}_{aml_key}", parent_line_id=parent_line_id, markup=td_invoice),
                'name': f'FACTURA {td_invoice}',
                'columns': [],
                'level': 2,
                'td_invoice': td_invoice
            }
            ...
            for col in columns:
                ...
                if col['expression_label'] == 'balance':
                    ...
                    column_copy['name'] = ''  # Mantener las otras columnas vacías
